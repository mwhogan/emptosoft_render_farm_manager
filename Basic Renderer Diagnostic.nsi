OutFile "Diagnostic Mode Client.exe"
Caption "ERFM (Beta) Client (Diagnostic Mode)"
...
VIAddVersionKey /LANG=1033-ENGLISH "FileDescription" "Emptosoft Render Farm Manager Client (Diagnostic Mode) (Version 0.4 - Beta)"
...
Render:
   ...
   FileOpen $TempData "$EXEDIR\T2TP-$JSNum-$FrameNum.bat" w
   FileWrite $TempData "set TERRAGEN_PATH=$TGLocS"
   FileWriteByte $TempData "13"
   FileWriteByte $TempData "10"
   FileWrite $TempData '"$TGLoc" -p "$World" -hide -exit -r -f $R0'
   FileWriteByte $TempData "13"
   FileWriteByte $TempData "10"
   FileWrite $TempData "Pause"
   FileClose $TempData
   ...
   SetDetailsPrint None
   StrCpy $Rendering "1"
   ExecPri::ExecWait "$EXEDIR\T2TP-$JSNum-$FrameNum.bat" "$Priority"
   Pop $TempData
   SetDetailsPrint Both
